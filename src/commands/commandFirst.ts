...
export async function executeCommand(args: any, messageInfo: any, resources: any): Promise<void> { 
    ...
    setTimeout(() => {
        ...
        publisher.publish<Command>(messageInfo.queueName, new Command('commandFirst', recordset[messageInfo.deliveryTag % recordset.length]))
        //1 publisher.publish<Command>(messageInfo.queueName, new Command('commandFirst', {a: `${args.a}`, n: `${n}`}))
    }, 1000);    
}
